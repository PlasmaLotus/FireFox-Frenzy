...
void AOEProjectileDrawable::setDisplayHitboxes(bool display)
{
...
{
	if (projectile->m_mainProjectileActive) {
		target.draw(m_sprite);
	}
	for (int i = projectile->m_projectiles.size() - 1; i >= 0; --i) {
		Projectile& p{ projectile->m_projectiles.at(i) };
		sf::CircleShape c;
...
bool AOEProjectileDrawable::onLoad()
{
	return true;
}
